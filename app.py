...
def generate_telaio(distanzeCampate: list, altezzePiani: list, E: float, A: float, I: float):
    nodes = []
    elements = []
    
    x_cum = [0.0]
    for d in distanzeCampate:
        x_cum.append(x_cum[-1] + d)
    
    y_cum = [0.0]
    for h in altezzePiani:
        y_cum.append(y_cum[-1] + h)
    ...
    for ix in range(len(x_cum)):
        for iy in range(len(y_cum) - 1):
            n1 = node_map[(iy, ix)]
            n2 = node_map[(iy + 1, ix)]
            elements.append({"id": elem_id, "n1": n1, "n2": n2, "prop": 1, "type": "beam2d"})
            pilastri_ids.append(elem_id)
            elem_id += 1
    
    for iy in range(1, len(y_cum)):
    ...
    for tid in travi_ids:
        dist_loads.append({"load_case_id": 1, "elem_id": tid, "qx0": 0.0, "qx1": 0.0, "qy0": qy, "qy1": qy})
    dist_loads_df = pd.DataFrame(dist_loads)
    
    nodes_df = pd.DataFrame(nodes)
    elements_df = pd.DataFrame(elements)
    properties_df = pd.DataFrame([
        {"id": 1, "name": "cls_30", "E": E, "A": A, "I": I, "rho": 2.5, "alphaT": 1.0e-5},
    ])
    
    restraints = []
    for ix in range(len(x_cum)):
        n = node_map[(0, ix)]
        restraints.append({"load_case_id": 1, "node_id": n, "ux": 1, "uy": 1, "rz": 1})
...
with st.sidebar:
    st.header("Generatore Telaio")
    
    st.subheader("Geometry")
    campate_str = st.text_input("Distanze campate (m)", "3.0, 4.0, 3.0", help="es: 3.0, 4.0, 3.0")
    piani_str = st.text_input("Altezze piani (m)", "3.5, 3.0, 3.0", help="es: 3.5, 3.0, 3.0")
    ...
    with col2:
        I = st.number_input("I (m4)", value=0.0225)
    
    if st.button("Genera Telaio"):
        try:
            distanze = [float(x.strip()) for x in campate_str.split(",") if x.strip()]
            altezze = [float(x.strip()) for x in piani_str.split(",") if x.strip()]
            sheets = generate_telaio(distanze, altezze, E, A, I)
            sheets["load_cases"] = pd.DataFrame([{"id": 1, "name": "permanente", "ax": 0.0, "ay": -9.81}])
    ...
    if st.button("📂 Carica esempio predefinito"):
        sheets = generate_telaio([3.0, 4.0, 3.0], [3.5, 3.0, 3.0], 30000.0, 0.3, 0.0225)
        sheets["load_cases"] = pd.DataFrame([{"id": 1, "name": "permanente", "ax": 0.0, "ay": -9.81}])
        sheets["node_loads"] = pd.DataFrame()
        st.session_state.sheets = ensure_sheets(sheets)
        st.session_state.results = None
        ...
        if nodes is None or nodes.empty or elems is None or elems.empty:
            ...
            if restraints is not None and not restraints.empty:
                for _, r in restraints.iterrows():
                    ...
                    rz = int(r.get("rz", 0))
                    if ux and uy and rz:
                        fig.add_trace(go.Scatter(x=[x], y=[y], mode="markers", marker=dict(symbol="square", size=14, color="red"), name="Incastro"))
                    elif ux and uy:
                        fig.add_trace(go.Scatter(x=[x], y=[y], mode="markers", marker=dict(symbol="triangle-up", size=12, color="blue"), name="Appoggio"))
                    else:
                        fig.add_trace(go.Scatter(x=[x], y=[y], mode="markers", marker=dict(symbol="circle-open", size=10, color="green"), name="Cerniera"))
            ...
            if dist_loads is not None and not dist_loads.empty:
                for _, dl in dist_loads.iterrows():
                    ...
                    if qy0 < 0:
                        fig.add_trace(go.Scatter(x=[xm], y=[ym], mode="markers+text", marker=dict(symbol="arrow-down", size=15, color="orange"), text=f"{abs(qy0):.1f}", textposition="top center", name="Carico distribuito"))
                    elif qy0 > 0:
                        fig.add_trace(go.Scatter(x=[xm], y=[ym], mode="markers+text", marker=dict(symbol="arrow-up", size=15, color="orange"), text=f"{abs(qy0):.1f}", textposition="bottom center", name="Carico distribuito"))
            ...
            if node_loads is not None and not node_loads.empty:
                for _, nl in node_loads.iterrows():
                    ...
                    x, y = coords[nid]
                    if fy != 0:
                        direction = "arrow-down" if fy < 0 else "arrow-up"
                        fig.add_trace(go.Scatter(x=[x], y=[y], mode="markers+text", marker=dict(symbol=direction, size=15, color="purple"), text=f"{fy:.1f}", textposition="top center", name="Carico nodale"))
